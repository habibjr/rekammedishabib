...
class TotalPasien extends StatefulWidget {
  TotalPasien({Key? key}) : super(key: key);


  @override
  State<TotalPasien> createState() => _TotalPasienState();
}

class _TotalPasienState extends State<TotalPasien> {
  ...
  void getData() async {
    try {
      ...
      Dio.Response response = await dio().get('/getdata',options: Dio.Options(headers: {'Authorization' : 'Bearer $token'}));
        ...
        appBar: AppBar(
          ...
          title:Row(
            children: [
              ...
              SizedBox(width: 9,),
              Text('Pasien List',style: GoogleFonts.nunito(color: Colors.white)),
              Spacer(),
              IconButton(
                  onPressed: (){
